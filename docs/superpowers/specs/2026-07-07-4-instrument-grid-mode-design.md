...
- MIDI/keyboard triggering logic in `main.js` (the `noteOn` handler and `setupKeyboard()`) dispatches through a shared `activeInstrumentNames` Set — populated with all 16 instrument names in 16-instrument mode, or just `['kick', 'snare', 'closedHihat', 'openHihat']` in 4-instrument mode — checked before every `drumSynth`/`visualizer.triggerNote` call. This was added after an initial version of this feature shipped without it: `note % padCount` always produces a value in `0..padCount-1` (it can never "fall outside" that range), so without this gate, a hidden instrument's note would silently alias onto one of the 4 visible cells and produce both sound and an incorrect visual reaction — the opposite of the "hide from GUI and don't react" behavior confirmed during design. `activeInstrumentNames` is updated by `applyGridMode` (in `setupGUI()`) every time Grid Mode changes.
